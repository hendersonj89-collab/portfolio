...
CUTS = [11.0, 33.0, 58.0, 86.0, 111.0, 131.0, 153.0]   # must match SCENES in render_harvest.py
...
PROG = [("Fmaj7", [53, 57, 60, 64]), ("Am7", [57, 60, 64, 67]), ("Dm7", [50, 57, 60, 65]), ("Bbmaj7", [46, 53, 57, 62])] * 5 + [("Fmaj7", [53, 57, 60, 64]), ("Fmaj7", [53, 57, 60, 64])]
CHORD_LEN = 7.5
...
for ci, (name, tones) in enumerate(PROG):
    ...
    level = 0.7 if t0 < 11 else 0.8 if t0 < 58 else 0.9 if t0 < 131 else 1.0
    if t0 >= 153: level = 0.7
    bright = 0.8 if t0 < 33 else 1.0 if t0 < 131 else 1.15
...
for beat in range(int(DUR / BEAT)):
    ...
    if 33 <= t < 153:
        for s in (0, 0.5):
            music.add(pulse(), t + s * BEAT, pan=0.2 if s else -0.2, gain=0.09 * (0.6 if t < 86 else 1.0) * (1.0 if s == 0 else 0.55))
    if 86 <= t < 153 and beat % 4 == 0:
        music.add(taiko(1.0 if t >= 131 else 0.7), t, gain=0.42)
    if 131 <= t < 153 and beat % 4 == 2:
        music.add(taiko(0.5), t, gain=0.3)
# rolls into the later cuts
for c in (111.0, 131.0, 153.0):
    for k in range(8):
        tt = c - 1.6 + k * 0.2
...
for (b, ln, m) in CELLO:
    music.add(cello(hz(m), ln * BEAT), 131.0 + b * BEAT, pan=-0.15, gain=0.13)
...
for c in CUTS:
    ...
    fx.add(boom(), c - 0.05, gain=0.32 if c < 153 else 0.45)
